...
/// Maximum number of stories a user is allowed to save.
...
fs.Value _convertValue(dynamic value) {
  if (value is String) {
    ...
    return fs.Value(arrayValue: fs.ArrayValue(values: value.map((e) => _convertValue(e)).toList()));
  } else if (value is Map) {
    return fs.Value(mapValue: fs.MapValue(fields: convertToFirestoreFields(value as Map<String, dynamic>)));
...
/// Limits the number of stories per user to [maxSaveStories].
Future<void> saveStory(String userId, Map<String, dynamic> storyJson) async {
  ...
  final String parentPath = "projects/$projectId/databases/(default)/documents";
  ...
  final String documentPath = "$parentPath/saved_stories/$documentId";
